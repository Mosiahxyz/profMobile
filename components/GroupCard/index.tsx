...
import { apiConfig } from '@/api/axios'; // Certifique-se de que o apiConfig esteja configurado corretamente
...
const deleteGroup = async (id: number, title: string, setModelos: React.Dispatch<React.SetStateAction<Modelo[]>>) => {
    Alert.alert(
        "Confirmação de Exclusão",
        `Tem certeza que deseja excluir o grupo: ${title}?`,
        [
            {
                text: "Cancelar",
                style: "cancel"
            },
            {
                ...
                onPress: async () => {
                    try {
                        // Faz a requisição DELETE usando apiConfig
                        const response = await apiConfig.delete(`/deletar/${id}`);
                        if (response.status === 200) {
                            Alert.alert('Sucesso', 'Item excluído com sucesso!');
                            // Remove o item excluído da lista localmente
                            setModelos(prevModelos => prevModelos.filter(modelo => modelo.id !== id));
                        } else {
                            Alert.alert('Erro', 'Falha ao excluir o item. Tente novamente.');
                        }
                    } catch (error) {
...
export default function GroupCard() {
    ...
    const fetchModelos = async () => {
        try {
            ...
            console.log('Modelos recebidos:', response.data);
                    ...
                    <Container key={modelo.id}>
                        ...
                        <TextSection>
                            ...
                            <ButtonContainer>
                                ...
                                <CustomButton onPress={() => deleteGroup(modelo.id, modelo.titulo, setModelos)}>
